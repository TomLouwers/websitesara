#!/usr/bin/env python3
"""
Update script voor procenten-vragen in verhaaltjessommen
...
def add_reflection_question(foutanalyse, error_type):
    ...
    reflections = {
        "conversiefout": "Hoe zet je een percentage om naar een breuk of decimaal getal?",
        "leesfout_ruis": "Welk bedrag of getal is het totaal (100%) en welk percentage moet je berekenen?",
        "conceptfout": "Welke formule gebruik je: percentage van totaal, of totaal berekenen uit percentage?",
        "rekenfout_basis": "Controleer je berekening: heb je correct vermenigvuldigd of gedeeld?"
    }

    reflection = reflections.get(error_type, "Wat ging er fout bij het berekenen van het percentage?")
...
def determine_error_type(foutanalyse, option_text):
    ...
    # Conversie fouten
    if any(word in foutanalyse_lower for word in ['omzetten', 'omrekenen', 'breuk', 'decimaal', '%', 'procent']):
...
def generate_hint(question_text, content):
    ...
    # Specifieke hints voor percentage vragen
    if "korting" in question_lower:
        return "💡 Tip: Korting betekent een percentage aftrekken van de originele prijs!"
    elif "hoeveel procent" in question_lower:
        return "💡 Tip: Deel het deel door het geheel en vermenigvuldig met 100!"
    elif "btw" in question_lower:
        return "💡 Tip: BTW tel je op bij de prijs. 21% BTW = prijs × 1,21!"
    elif "rente" in question_lower:
        return "💡 Tip: Rente = percentage van het startbedrag!"
    elif "hoeveel is" in question_lower and "%" in question_text:
        return "💡 Tip: Percentage van een bedrag = bedrag × (percentage ÷ 100)!"
    elif "groeide" in question_lower or "steeg" in question_lower or "daalde" in question_lower:
        return "💡 Tip: Bij groei/daling: bereken het percentage en tel op/trek af!"
    elif "wat was" in question_lower or "oorspronkelijk" in question_lower:
        return "💡 Tip: Als je het nieuwe bedrag hebt, werk dan terug naar het origineel!"
    else:
        return "💡 Tip: Denk goed na: bereken je een percentage VAN een bedrag, of een bedrag MET percentage?"
...
def main():
    ...
    print(f"🔍 Gevonden: {procenten_count} procenten-items")
    ...
    print("✅ Klaar! Alle procenten-vragen zijn geüpdatet.")
