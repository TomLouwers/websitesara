#!/usr/bin/env python3
"""
Update script voor tijd-snelheid-vragen in verhaaltjessommen
...
def add_reflection_question(foutanalyse, error_type):
    ...
    reflections = {
        "conversiefout": "Welke eenheid moet je omrekenen (uur↔minuten of km↔meter)?",
        "leesfout_ruis": "Welke informatie over snelheid en tijd is belangrijk?",
        "conceptfout": "Welke formule gebruik je: afstand = snelheid × tijd?",
...
def generate_hint(question_text, content):
    ...
    # Specifieke hints voor tijd-snelheid vragen
    if "snelheid" in question_lower and ("berekenen" in question_lower or "wat is" in question_lower):
        return "💡 Tip: Snelheid = afstand ÷ tijd"
    elif "afstand" in question_lower and ("berekenen" in question_lower or "hoeveel" in question_lower):
        return "💡 Tip: Afstand = snelheid × tijd"
    elif "tijd" in question_lower and ("totaal" in question_lower or "samen" in question_lower):
        return "💡 Tip: Tel alle tijden bij elkaar op!"
    elif "minuten" in question_lower or "minuut" in question_lower:
        return "💡 Tip: Let op! Reken minuten om naar uren als dat nodig is."
    elif "uur" in question_lower and "minuten" in question_lower:
    ...
    elif "sneller" in question_lower or "langzamer" in question_lower:
        return "💡 Tip: Vergelijk de snelheden door de afstanden en tijden te gebruiken!"
    else:
        return "💡 Tip: Denk na over de relatie tussen tijd, snelheid en afstand"

def update_question(question, theme):
    """Update een enkele vraag met nieuwe velden"""
    if theme != "tijd-snelheid":
        return question
...
def main():
    ...
    # Update alle tijd-snelheid-vragen
    tijd_count = 0
    updated_count = 0
    ...
    print(f"🔍 Gevonden: {tijd_count} tijd-snelheid-items")
    ...
    print("✅ Klaar! Alle tijd-snelheid-vragen zijn geüpdatet.")
