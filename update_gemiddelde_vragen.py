#!/usr/bin/env python3
"""
Update script voor gemiddelde-vragen in verhaaltjessommen
...
def add_reflection_question(foutanalyse, error_type):
    ...
    reflections = {
        "conversiefout": "Welke eenheid moet je gebruiken voor het gemiddelde?",
        "leesfout_ruis": "Welke getallen moet je optellen en door hoeveel moet je delen?",
        "conceptfout": "Wat is de formule voor gemiddelde: (som van alle getallen) ÷ (aantal getallen)?",
        "rekenfout_basis": "Controleer je optelling en deling nog eens!"
    }

    reflection = reflections.get(error_type, "Wat ging er fout bij het berekenen van het gemiddelde?")
...
def determine_error_type(foutanalyse, option_text):
    ...
    # Conversie fouten
    if any(word in foutanalyse_lower for word in ['omrekenen', 'eenheid', 'uur', 'minuten', 'meter', 'centimeter']):
        return "conversiefout"

    # Lees fouten / ruis
    if any(word in foutanalyse_lower for word in ['vergeten', 'over het hoofd', 'niet gelezen', 'genegeerd', 'gemist', 'aantal']):
        return "leesfout_ruis"

    # Concept fouten
    if any(word in foutanalyse_lower for word in ['verkeerde', 'concept', 'methode', 'aanpak', 'strategie', 'denkfout', 'formule', 'delen']):
        return "conceptfout"

    # Reken fouten
    if any(word in foutanalyse_lower for word in ['rekenfout', 'controleer', 'berekening', 'optellen', 'som', 'delen', 'deling']):
...
def generate_hint(question_text, content):
    ...
    # Specifieke hints voor gemiddelde vragen
    if "gemiddelde" in question_lower and ("cijfer" in question_lower or "punt" in question_lower):
        return "💡 Tip: Tel alle cijfers op en deel door het aantal toetsen!"
    elif "gemiddelde snelheid" in question_lower:
        return "💡 Tip: Gemiddelde snelheid = totale afstand ÷ totale tijd!"
    elif "gemiddelde" in question_lower and "temperatuur" in question_lower:
        return "💡 Tip: Tel alle temperaturen op en deel door het aantal metingen!"
    elif "gemiddelde" in question_lower and ("leeftijd" in question_lower or "jaar" in question_lower):
        return "💡 Tip: Tel alle leeftijden op en deel door het aantal personen!"
    elif "gemiddelde" in question_lower and ("prijs" in question_lower or "euro" in question_lower or "kost" in question_lower):
        return "💡 Tip: Tel alle prijzen op en deel door het aantal items!"
    elif "hoeveel" in question_lower and ("nodig" in question_lower or "moet" in question_lower):
        return "💡 Tip: Bereken eerst de som die je nodig hebt, trek af wat je al hebt!"
    else:
        return "💡 Tip: Gemiddelde = (som van alle getallen) ÷ (aantal getallen)!"
...
def main():
    ...
    print(f"🔍 Gevonden: {gemiddelde_count} gemiddelde-items")
    ...
    print("✅ Klaar! Alle gemiddelde-vragen zijn geüpdatet.")
